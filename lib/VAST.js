...
VAST.fetch = function fetch(uri/*, options, callback*/) {
    ...
    var resolvedPromise;
    if (uri && uri.lastIndexOf('data:') === 0) {
        resolvedPromise = LiePromise.resolve(decodeURIComponent(uri.substr(uri.indexOf(',') + 1)));
    } else {
        resolvedPromise = LiePromise.resolve(request.get(uri).set(options.headers || {}))
            .then(function(response) { return response.text;});
    }
    return nodeifyPromise(resolvedPromise
        .then(function makeVAST(text) {
            var vast = new VAST(VAST.pojoFromXML(text));
