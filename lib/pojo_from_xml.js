...
function parseLinear(creative) {
    ...
    var linear = creative.find('Linear');
    ...
    return {
        ...
        skipoffset: linear[0].attributes.skipoffset
    };
}
...
module.exports = function pojoFromXML(xml) {
    ...
    return trimObject({
        ...
        ads: $('Ad').map(function(ad) {
            var wrapperOrInline = ad.find('Wrapper,InLine');
            if (!wrapperOrInline) {
                return null;
            }
            var type = single(wrapperOrInline).tag.toLowerCase();
